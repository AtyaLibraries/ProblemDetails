...
### Result details and 5xx security

Result errors mapped to statuses from 500 through 599 use the same secure-detail
policy as exception errors. By default, `detail` is `An unexpected error
occurred.` and error-derived extensions such as the error code and nested
validation errors are omitted. Trace and correlation identifiers remain
available because they are generated by the request pipeline rather than copied
from the error.

Non-5xx mappings retain their existing behavior: the error message is used as
`detail`, and configured error-code and validation extensions are emitted.

An application may intentionally disclose one reviewed Result detail by using a
`MapError` detail factory. The factory controls only `detail`; error-derived
extensions remain redacted for 5xx responses.
...
builder.Services.AddAtyaProblemDetails(options =>
{
    options.MapError(
        ...
        "urn:example:problem-type:internal-server-error",
        static (error, _) => error.Message); // Review before enabling.
});
```

This is a behavioral compatibility change from version 1.1.0 and earlier,
which could copy Result error content into a 5xx response. Migration normally
requires no code change. If a client has an approved dependency on a public 5xx
detail, configure a narrow `MapError` factory that returns only that documented
content. To roll back before publication, revert the isolated policy change. To
restore a reviewed legacy detail after publication, use the explicit mapping
above while preparing a forward package update; published packages and tags
